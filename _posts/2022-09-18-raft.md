...
Paxos算法，尽管Paxos在分布式算法的地位很重要，但是因其晦涩难懂且缺乏工程实现，因此出现了不少取而代之的算法，今天我们就来分析一种和 Paxos有异曲同工之妙的的共识算法 Raft。
...
在讲解 Leader选举之前，我们先了解下 选举约束、任期、随机超时、通信方式 等几个基本概率，帮助后面更好的去理解 Leader选举机制。

### 3.1 选举约束

1. 同一任期内每个节点最多只能投票 1次，并且按照 first-come-first-served(先来先服务) 的原则。
2. Candidate(候选人) 只有获得集群中大多数选票才能成为 Leader(领导者)。
3. 日志完整性高的 Follower(跟随者)拒绝投票给日志完整性低的 Candidate(候选人)，这里的日志指的是已复制未commit状态。也就是说，即便 Candidate(候选人)的 term 大于 Follower(跟随者)的 term，假如 Candidate(候选人) 向 Follower(跟随者)发送了一条投票RPC，如果当前消息中的term 小于 Follower(跟随者)最后一条消息的 term，则 Follower(跟随者) 拒绝给 Candidate(候选人)投票。
...
Raft算法中节点之间采用 RPC 进行通信，这里包含两种类型的 RPC：
...
有了上面几个基本概念的铺垫，为了更情景化的说明 Leader的选举过程，本文将以包含 节点A、节点B、节点C 3个节点组成的集群来进行演示。
...
这里以[A,B,C,D,E] 5个节点组成的集群为例，假如内部出现了网络问题，节点[A,B,C]为一个分区，节点[D,E]为一个分区，节点A获得[A,B,C]分区的大多数选票成为 Leader，节点D获得[D,E]分区的大多数选票成为 Leader，因此一个集群产生了 2个Leader，这就是我们常说的"脑裂问题"。

Raft是如何解决这种脑裂问题？

> 答案：当网络恢复正常后，两个分区的 Leader都会向其他节点发送心跳，当节点A收到节点B的心跳之后，发现B的任期比自己大，因此节点A恢复成Follower，因此整个集群就恢复成只有一个 Leader的状态。
...
在讲解 log replication(如何复制日志)之前，我们同样需要先申明几个约束：

1. 日志复制 RPC 只能由 Leader(领导者)发起
2. Leader(领导者) 一定包含最新的committed log，而且从不修改或删除自己的日志项
3. 一个log被复制到大多数节点后，状态就是committed，保证不会回滚
4. 不同节点，如果某个位置(索引)上日志相同，那么这个位置之前的所有日志也一定相同


### 4.1 什么是日志项/日志条目

日志项/日志条目 是组成日志的数据单元，它是一种数据格式，主要包含索引值(Log index)、任期编号(Term)、指令(Command) 三部分，如下图(摘自官方)：

![img.png](https://yuanjava.cn/assets/md/framework/raft-log.png)

- 索引值：日志项对应的整数索引值，它是用来标识日志项的，是一个连续单调递增的整数；
- 任期编号：创建这条日志项的 Leader(领导者)的任期编号;
...
- Leader(领导者) 接收到客户端请求后，创建一个 new entry(新日志项)，并 appends(追加)到本地日志中(Leader的日志项为uncommitted状态)；
- Leader(领导者) 以同步的方式向所有 Follower(跟随者) 发送 AppendEntries RPC 日志项复制请求(Follower的日志项为uncommitted状态)；
- Leader(领导者) 得到 major(大多数) Follower(跟随者)的复制成功的响应后，Leader(领导者)将日志项应用到它的状态机中(Leader的日志项为committed状态)；
- Leader(领导者) 将执行的结果返回给客户端；
- Leader(领导者) 通过心跳或新的 AppendEntries RPC 将提交了某条日志项的状态同步给Follower(跟随者)，Follower(跟随者)将日志项状态同步到本地状态机中(Follower的日志项为committed状态)；
- 如果 Follower(跟随者)出现崩溃、运行缓慢、网络丢包，Leader(领导者)会不断地重试 AppendEntries RPCs（即使已经对客户端作出了响应）直到所有的 Follower(跟随者)成功存储了所有的日志项；
...
1. Leader(领导者) 通过日志复制 RPC 的**一致性检查**，找到 Follower(跟随者)节点上，与自己具有相同日志项的最大 index索引值；
2. Leader(领导者) 强制 Follower(跟随者) 更新覆盖的不一致日志项，实现日志的一致；

怎么理解呢？这里以一个实例来进行讲解，如下图：

![img.png](https://yuanjava.cn/assets/md/framework/raft-log-inconsistent.png)

图中包含了 1个 Leader 和 1个 Follower的所有日志项，整个复制过程分以下几个步骤(步骤1-4是一致性检查机制)：

1. Leader(领导者) 当前最大日志项索引是 10，因此 Leader(领导者) 会通过日志复制 RPC 消息将 index=9 的日志发送给 Follower(跟随者)，Follower(跟随者) 判断自己没有index=9的日志，因此拒绝更新日志并响应 Leader 失败信息。
2. Leader(领导者) 收到 Follower(跟随者) 的失败响应后，执行index-1，将 index=8的日志发送给 Follower(跟随者)，Follower(跟随者) 判断自己index=8日志项信息为term=4,x->7，和 Leader(领导则)日志项不相同 ，因此再次拒绝更新，响应 Leader失败信息。
3. Leader(领导者) 收到 Follower 的失败响应后，重复操作上述过程，直到 index=6;
4. Leader(领导者) 将 index=6的日志发送给 Follower(跟随者)，Follower判断自己 index=6 日志项中的 term和command 和 Leader相同，响应日志复制成功。因此，Leader(领导者)就知道在 index=6「term=3,y->1」日志项位置，Follower(跟随者)的日志项与自己相同。
5. Leader(领导者) 通过日志复制 RPC消息，强制 Follower(跟随者)复制并更新覆盖 index=6之后的所有日志项(不一致的日志项)，达到 Follower 与 Leader的日志保持一致；
6. 集群中多个 Follower(跟随者)，只需要重复上述过程，就能最终实现了集群各节点日志的一致。

### 4.4 常见问题

日志复制过程中，对于客户端的command X指令，如果 Leader(领导者) 成功响应了客户端(步骤4)，但 Follower(跟随者)还未commit(步骤5)，Leader(领导者)突然挂了，新 Leader(领导者) 如何保证 command X指令在其他节点commit？

```text
答案：Leader(领导者)选举能保证新 Leader(领导者)一定包含这条committed的日志项，但这条日志项在新 Leader(领导者)上处于未提交状态，这时新Leader(领导者)，会尝试从最新日志项开始，向其他节点复制日志，如果，某条uncommitted的日志项，被发现已经成功复制到大多数节点上，这时这条日志项将处于提交状态，并被应用到状态机，通知其他节点提交这条日志。
```

为什么 Follower(跟随者) 不直接告诉 Leader(领导者) 从哪个index开始日志缺页了，而需要 Leader(领导者)一个一个去尝试，重复RPC操作，消耗网络资源？

```text
答案：因为相同的index索引，可能来自不同的term任期，所以 Leader(领导者)需要从最大的index 往前一个个比较相同 index 的 Follower的日志项，直到找到第一「term和command」相同的日志项，然后将后面的日志全部覆盖更新。
```
...
>
> 首先，Leader(领导者) 创建 Cold,new 日志项，并复制到新旧配置中的大多数，此时所有的日志项都需要被联合共识。
>
> 然后，Leader(领导者) 创建 Cnew 日志项，并复制到 Cnew(新配置)中的大多数。因此，旧配置和新配置不会存在可以同时做出决策的时间点。
...
> 对于新的节点D、E，Raft 会通过日志一致性检查来复制领导者的所有日志项，从而保证它们同样能够保持日志完整性。
...
- 第二步，Leader(领导者)节点A 将新配置[A, B, C, D]作为一个日志项，复制到新配置中所有节点(节点 A、B、C、D)上，然后将新配置的日志项应用（Apply）到本地状态机，完成单节点变更。
...
- 第二步，Leader(领导者)节点A 将新配置[A, B, C, D, E]作为一个日志项，复制到新配置中所有节点(节点 A、B、C、D、E)上，然后将新配置的日志项应用（Apply）到本地状态机，完成单节点变更。
