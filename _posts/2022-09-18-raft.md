...
Paxos算法，尽管Paxos在分布式算法的地位很重要，但是因其晦涩难懂且缺乏工程实现，因此出现了不少取而代之的算法，今天我们就来分析一种和 Paxos有异曲同工之妙的的共识算法 Raft。 内容大纲如下：
...
在讲解 Leader选举之前，我们先了解 任期、随机超时、通信方式 等几个基本概率，帮助后面更好的去理解 Leader选举机制。
...
```text
答案：Leader(领导者)选举能保证新 Leader(领导者)一定包含这条committed的日志条目，但这条日志条目在新 Leader(领导者)上处于未提交状态，这时新Leader(领导者)，会尝试从最新日志条目开始，向其他节点复制日志，如果，某条uncommitted的日志条目，被发现已经成功复制到大多数节点上，这时这条日志条目将处于提交状态，并被应用到状态机，通知其他节点提交这条日志。
```
