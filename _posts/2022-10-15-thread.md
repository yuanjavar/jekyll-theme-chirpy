---
layout: post
title: Java 线程，那个似懂非懂的概念，程序员必看！
category: java
tags: [java,interview]
excerpt:  Java 线程，那个似懂非懂的概念，程序员必看！
...
> 申明：本文基于 jdk-11.0.15，操作系统基于 Linux，JVM 基于hotspot源码，[hotspot 源码下载地址](https://github.com/openjdk/jdk)
...
想必大家很自然就会想到 Thread.stop()，不过，很遗憾的是，Thread.stop() 从JDK 1.2就已经被废弃了，主要是因为该方法本质上是不安全的，使用它来停止线程会导致所有已锁定的监视器都被解锁，因此推荐使用 Thread.interrupt()来中断线程。
...
接下来分析 Thread.interrupt()是如何中断线程的。

在 Thread源码中，interrupt() 最终调用了 native interrupt0()，从而转向了 JVM的实现

![img.png](https://yuanjava.cn/assets/md/java/java-interrupt.png)

在 JVM Thread.c源码中，我们可以看到 interrupt0()映射到 JVM_Interrupt方法

![img.png](https://yuanjava.cn/assets/md/java/jvm-interrupt0.png)

在 jvm.cpp源码中 JVM_Interrupt实现为，判断线程是否存活，如果存活，则调用 JavaThread.interrupt()进行线程中断操作：

![img.png](https://yuanjava.cn/assets/md/java/JVM_Interrupt.png)

在 javaThread.cpp 中，interrupt()的实现逻辑如下：
